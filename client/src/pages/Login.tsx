    ...
    try {
      ...
      const message =
        err?.message?.includes("popup-closed-by-user")
          ...
          : err?.message?.includes("operation-not-allowed") || err?.message?.includes("not enabled")
          ? "Facebook sign-in is not yet enabled. Please use Google or email."
    ...
    try {
      ...
      const message =
        err?.message?.includes("popup-closed-by-user")
          ? "Sign-in was cancelled."
          : err?.message?.includes("operation-not-allowed") || err?.message?.includes("not enabled")
          ? "Apple sign-in is not yet enabled. Please use Google or email."
          ...
          <div className="w-full" data-testid="panel-form">
            ...
            >
              ...
              <div className="mb-8 space-y-2.5" data-testid="social-login-section">
                ...
                {/* 2-column row: Facebook + Apple */}
                <div className="grid grid-cols-2 gap-2.5">
                  <button
                    onClick={handleFacebookLogin}
                    disabled={isLoading || isGoogleLoading}
                    className="flex items-center justify-center gap-2 px-3 py-3 rounded-xl border border-[#1877F2]/40 bg-[#1877F2]/5 hover:bg-[#1877F2]/15 hover:border-[#1877F2]/60 transition-all text-slate-100 font-semibold active:scale-[0.98] text-sm"
                  ...
                  <button
                    ...
                    disabled={isLoading || isGoogleLoading}
                    className="flex items-center justify-center gap-2 px-3 py-3 rounded-xl border border-slate-600 bg-white/[0.03] hover:bg-white/[0.08] hover:border-slate-400 transition-all text-slate-100 font-semibold active:scale-[0.98] text-sm"
