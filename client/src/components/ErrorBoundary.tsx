...
interface Props {
  children: ReactNode;
  fallback?: ReactNode;
  /** Compact inline fallback — used for section-level boundaries */
  inline?: boolean;
}
      ...
      <div className="max-w-md w-full text-center space-y-6">
        ...
        <div className="space-y-2">
          <h2 className="text-2xl font-bold text-white">
            {isChunk ? "New version available" : isNet ? "Connection lost" : "Something went wrong"}
          </h2>
          <p className="text-slate-400 text-sm leading-relaxed">
            {isChunk
              ? "FreelanceSkills just deployed an update. Refresh to get the latest version."
              : isNet
        ...
        {import.meta.env.DEV && error && (
          <details className="text-left mt-4">
            <summary className="text-xs text-slate-500 cursor-pointer">Dev: error details</summary>
            <pre className="mt-2 text-xs text-red-400 bg-slate-900 p-3 rounded-lg overflow-auto max-h-40 border border-red-500/20">
              {error.message}
              {"\n"}
              {error.stack}
            </pre>
...
function InlineFallback({ onReset }: { onReset(): void }) {
  return (
    <div
      className="flex items-center justify-between gap-3 p-4 rounded-xl border border-red-500/20 bg-red-500/5 text-sm"
    ...
    >
      ...
      <Button size="sm" variant="ghost" onClick={onReset} className="gap-1 text-xs text-red-400 hover:text-red-300">
...
export class ErrorBoundary extends Component<Props, State> {
  ...
  static getDerivedStateFromError(error: Error): State {
    return {
      hasError: true,
      error,
      errorType: isChunkError(error) ? "chunk" : isNetworkError(error) ? "network" : "generic",
    };
  }
...
/**
 * Thin wrapper for wrapping individual sections or cards.
 * Renders an inline retry chip instead of a full-screen error.
 */
