/**
 * ProfileReadinessGate — FreelanceSkills
 *
 * Shown INSTEAD OF SILENCE when a user tries to apply for a job
 * but their profile is incomplete or unpublished.
 *
 * Kills the confusion loop: shows EXACTLY what's missing,
 * WHY it's required, and a single-click path to fix it.
 */
...
interface ProfileReadinessGateProps {
  ...
  jobTitle?: string;
  onClose: () => void;
  /** Called only when readiness check confirms the user is ready to apply */
  onReady: () => void;
}
...
export function ProfileReadinessGate({ open, jobTitle, onClose, onReady }: ProfileReadinessGateProps) {
  ...
  // If actually ready, skip the gate
  if (data?.ready && open) {
    onReady();
    return null;
  }
  ...
  const goToProfile = () => {
    onClose();
    navigate(nextAction === "publish_profile" ? "/cv-upload" : "/cv-upload");
  };
  ...
  const publishProfile = async () => {
    try {
      ...
      if (res.ok) {
        onClose();
        // Re-open the apply flow after a short delay
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { if (!v) onClose(); }}>
      <DialogContent className="max-w-lg bg-slate-950 border border-slate-800 text-foreground p-0 overflow-hidden">
        {/* Header */}
        <div className="px-6 pt-6 pb-4 border-b border-slate-800 bg-slate-900/50">
          <DialogHeader>
            <DialogTitle className="text-lg font-bold text-white flex items-center gap-2">
              <Lock className="w-5 h-5 text-amber-400" />
              {jobTitle ? `Apply to "${jobTitle}"` : "Unlock Job Applications"}
            </DialogTitle>
          </DialogHeader>
          ...
          <div className="mt-4 flex items-center justify-between">
            ...
            <span className="text-xs font-semibold text-slate-400">{score}% complete</span>
          </div>
          <Progress
          ...
          {isLoading ? (
            ...
            <>
              ...
              <div className="flex items-start gap-3 p-4 rounded-xl bg-amber-500/5 border border-amber-500/20">
                ...
                <p className="text-sm text-amber-200 leading-relaxed">{data?.message}</p>
              </div>
              ...
              {criticalMissing.length > 0 && (
                <div className="space-y-2">
                  <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">
                    Required to apply ({criticalMissing.length} remaining)
                  </p>
                  <div className="space-y-1.5">
                    {criticalMissing.map((item) => {
                      const Icon = FIELD_ICONS[item.field] ?? AlertCircle;
                      return (
                        <div
                          key={item.field}
                          className="flex items-center gap-2.5 p-2.5 rounded-lg bg-red-500/5 border border-red-500/15"
                        >
                          <XCircle className="w-4 h-4 text-red-400 shrink-0" />
                          <Icon className="w-4 h-4 text-red-300/60 shrink-0" />
                          <span className="text-sm text-red-200">{item.label}</span>
                          <Badge variant="outline" className="ml-auto text-[10px] border-red-500/20 text-red-400">Required</Badge>
                        </div>
                      );
                    })}
                  </div>
              ...
              {missing.filter((m) => !m.critical).length > 0 && (
                <div className="space-y-2">
                  <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">
                    Boosts your application
                  </p>
                  <div className="space-y-1.5">
                    {missing.filter((m) => !m.critical).map((item) => {
                      const Icon = FIELD_ICONS[item.field] ?? AlertCircle;
                      return (
                        <div key={item.field} className="flex items-center gap-2.5 p-2.5 rounded-lg bg-slate-800/50 border border-slate-700/50">
                          <AlertCircle className="w-4 h-4 text-slate-500 shrink-0" />
                          <Icon className="w-4 h-4 text-slate-500/60 shrink-0" />
                          <span className="text-sm text-slate-400">{item.label}</span>
                          <Badge variant="outline" className="ml-auto text-[10px] border-slate-600 text-slate-500">Optional</Badge>
                        </div>
                      );
                    })}
                  </div>
              ...
              {completed.length > 0 && (
                <div className="space-y-2">
                  <p className="text-xs font-semibold text-emerald-500/70 uppercase tracking-wider">
                    Already done ✓
                  </p>
                  <div className="flex flex-wrap gap-2">
                    {completed.map((item) => (
                      <div key={item.field} className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-500/5 border border-emerald-500/15 text-xs text-emerald-400">
        ...
        <div className="px-6 pb-6 space-y-2">
          {nextAction === "publish_profile" ? (
            ...
            >
              ...
              Publish Profile Now — Unlock Applications
            ...
            >
              ...
              Complete Profile with AI — 60 seconds
              <ArrowRight className="w-4 h-4" />
            </Button>
          )}
          <Button
...
// ── Inline hook for job card apply buttons ────────────────────────────────────
// Returns a gate-aware apply handler. Usage:
//   const applyOrGate = useApplyWithReadinessGate(user, showAuthModal);
//   <button onClick={() => applyOrGate(job, openCoverLetter)} />
