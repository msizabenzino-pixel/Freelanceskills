...
async function aiInfer(system: string, user: string, temperature = 0.3): Promise<InferenceResult> {
  ...
  const resp = await fetch(`${AI_BASE()}/chat/completions`, {
    ...
    body: JSON.stringify({ model: "gpt-5-mini", messages: [{ role: "system", content: system }, { role: "user", content: user }], temperature }),
  });
  ...
  // gpt-5-mini: $0.000150/1K input · $0.000600/1K output
...
const AGENT_SEEDS = [
  { name: "ProposalRanker", specialization: "proposal_ranking", description: "Semantically ranks and scores freelancer proposals using multi-dimensional analysis: skills match, communication quality, cultural fit, Africa timezone compatibility, and budget alignment.", model: "gpt-5-mini", capabilities: ["semantic_scoring","skills_match","proposal_quality","budget_analysis"], africaOptimized: true },
  { name: "JobMatcher", specialization: "job_matching", description: "Hyper-personalized job-to-freelancer matching engine using collaborative filtering, skill graph embeddings, and Africa-first location intelligence.", model: "gpt-5-mini", capabilities: ["skill_graph","location_boost","historical_success","salary_fit"], africaOptimized: true },
  { name: "FraudDetector", specialization: "fraud_detection", description: "Multi-signal real-time scam and fraud detection. Analyzes behavioral patterns, device fingerprints, payment velocity, IP reputation, and Africa-specific mobile money fraud patterns.", model: "gpt-5-mini", capabilities: ["behavioral_analysis","velocity_check","ip_reputation","mobile_money_fraud","identity_graph"], africaOptimized: true },
  { name: "SupportBot", specialization: "support_chatbot", description: "Multi-turn contextual support chatbot with long-term memory. Speaks 8 African languages, understands USSD context, mobile money issues, and escalates to human agents when needed.", model: "gpt-5-mini", capabilities: ["multi_turn_memory","african_languages","ussd_context","escalation","empathy"], africaOptimized: true },
  { name: "ContentModerator", specialization: "content_moderation", description: "Multimodal content moderation engine. Scans text, image descriptions, and voice transcripts for fraud, hate speech, scam attempts, and platform policy violations.", model: "gpt-5-mini", capabilities: ["text_moderation","image_analysis","voice_transcript","scam_detection","policy_enforcement"], africaOptimized: false },
  { name: "SkillAdvisor", specialization: "skill_recommendations", description: "Academy course and skill-gap recommender. Analyzes freelancer profile, market demand trends, Africa job market data, and learning velocity to suggest highest-ROI courses.", model: "gpt-5-mini", capabilities: ["skill_gap_analysis","market_demand","course_recommendation","africa_job_market","learning_velocity"], africaOptimized: true },
  { name: "PriceOptimizer", specialization: "dynamic_pricing", description: "Dynamic pricing and promotion optimizer. Uses demand elasticity, Africa purchasing power parity, competitor analysis, and time-of-day patterns to maximize revenue and conversion.", model: "gpt-5-mini", capabilities: ["demand_elasticity","africa_ppp","competitor_analysis","promotion_timing","revenue_maximization"], africaOptimized: true },
  { name: "DisputePredictor", specialization: "dispute_prediction", description: "Early dispute risk prediction and prevention agent. Monitors communication sentiment, milestone progress, payment delays, and behavioral signals to intervene before disputes escalate.", model: "gpt-5-mini", capabilities: ["sentiment_monitoring","milestone_tracking","payment_delay","early_intervention","mediation_ready"], africaOptimized: false },
  { name: "ChurnPredictor", specialization: "churn_ltv", description: "User lifetime value and churn prediction engine. Builds individual LTV models, identifies at-risk users 30 days before churn, and recommends personalized retention actions.", model: "gpt-5-mini", capabilities: ["ltv_modeling","churn_risk_scoring","retention_playbook","cohort_analysis","africa_loyalty_patterns"], africaOptimized: true },
  { name: "NotificationEngine", specialization: "notification_personalization", description: "Hyper-personalized re-engagement and notification engine. Optimizes channel (WhatsApp/USSD/email/SMS/push), timing, copy, and incentive for each user segment.", model: "gpt-5-mini", capabilities: ["channel_optimization","send_time_optimization","copy_personalization","ussd_notifications","whatsapp_business"], africaOptimized: true },
  { name: "RedTeamSimulator", specialization: "adversarial_testing", description: "Adversarial scam simulator that generates realistic fraud patterns to stress-test the FraudDetector. Creates novel attack vectors from real Africa cybercrime patterns to keep detection ahead of attackers.", model: "gpt-5-mini", capabilities: ["attack_generation","novel_vector_creation","fraud_pattern_database","detector_training","africa_cybercrime"], africaOptimized: true },
  { name: "SwarmOrchestrator", specialization: "multi_agent_orchestration", description: "Master orchestrator that routes tasks to the right agent swarm, manages parallel inference, aggregates votes using weighted confidence, and logs all swarm decisions for audit.", model: "gpt-5-mini", capabilities: ["task_routing","parallel_inference","majority_voting","confidence_aggregation","swarm_audit"], africaOptimized: false },
];
...
export async function registerAiBrainRoutes(app: Express, isAuthenticated: any) {
  ...
  // ─── CREATE TABLES ──────────────────────────────────────────────────────────
  try {
    await db.execute(sql`CREATE TABLE IF NOT EXISTS ai_agents (id VARCHAR(36) PRIMARY KEY DEFAULT gen_random_uuid(), name VARCHAR(64) NOT NULL, specialization VARCHAR(64) NOT NULL, description TEXT, model VARCHAR(64) DEFAULT 'gpt-5-mini', status VARCHAR(16) DEFAULT 'online', health_score REAL DEFAULT 100, total_inferences INTEGER DEFAULT 0, avg_latency_ms REAL DEFAULT 0, avg_confidence REAL DEFAULT 0, total_tokens_used INTEGER DEFAULT 0, total_cost_usd REAL DEFAULT 0, capabilities JSONB DEFAULT '[]', africa_optimized BOOLEAN DEFAULT FALSE, is_active BOOLEAN DEFAULT TRUE, last_active_at TIMESTAMP DEFAULT NOW(), created_at TIMESTAMP DEFAULT NOW())`);
  ...
  app.post("/api/ai/chat", async (req: Request, res: Response) => {
    ...
    try {
      ...
      const start = Date.now();
      const resp = await fetch(`${AI_BASE()}/chat/completions`, { method: "POST", headers: { "Content-Type": "application/json", "Authorization": `Bearer ${AI_KEY()}` }, body: JSON.stringify({ model: "gpt-5-mini", messages: [{ role: "system", content: system }, ...messages], temperature: 0.7 }) });
      const latencyMs = Date.now() - start;
      if (!resp.ok) throw new Error("AI API " + resp.status);
      const data = await resp.json();
